...
class UserMetricsCacheBuilder:
    ...
    def build_cache(self, force_rebuild=False):
        ...
        if not force_rebuild and self._cache_exists():
            print(" 用户指标缓存已存在，跳过构建")
            return True
        
        print("\n" + "="*60)
        print("开始构建用户数据缓存（ISS指标 + 用户评论数据）")
        print("="*60)
        ...
        try:
            ...
            print(f"   加载了 {len(df)} 条评论，涉及 {df['user_id'].nunique()} 个用户")
            
            # 2. 按用户分组处理（优化版本）
            print("步骤2/3: 计算ISS指标并缓存用户评论数据...")
            iss_metrics = {}
            user_reviews_cache = {}
            ...
            for user_id, user_reviews in user_groups.items():
                processed += 1
                if processed % 5000 == 0:
                    print(f"  处理进度: {processed}/{total_users} ({100*processed/total_users:.1f}%)")
                # 计算ISS指标
            ...
            print(f"   完成 {len(iss_metrics)} 个用户的数据处理")
            ...
            print(f" 缓存文件已保存到: {self.cache_dir}")
            print(f"  - ISS指标缓存: {self.iss_cache_file}")
            print(f"  - 用户评论数据缓存: {self.user_reviews_cache_file}")
            print("="*60)
...
class UserMetricsCacheReader:
    ...
    def _load_cache(self):
        ...
        print(f" 用户数据缓存加载成功")
        print(f"  - ISS指标: {len(self.iss_metrics)} 个用户")
        print(f"  - 用户评论数据: {len(self.user_reviews)} 个用户")
...
if torch.cuda.is_available():
    ...
    print("将使用GPU进行主要计算和训练")
else:
    device = torch.device("cpu")  # 主要计算使用CPU
    gpu_device = None
    print("未检测到GPU，所有计算将使用CPU")
...
result_dir = None
print(f"结果将保存在 preprocessed_<数据集名>/full_data 目录中")
...
class Module1_NodeSplitting:
    ...
    def load_reviews_data(self):
        """从数据库加载评论数据"""
        print(f"\n[模块1] 开始加载评论数据...")
        print(f"[模块1] 数据库路径: {self.db_path}")
        print(f"[模块1] 采样比例: {self.sample_ratio}")
        ...
        print(f"[模块1] 加载完成，共 {len(df)} 条评论记录")
        print(f"[模块1] 涉及用户数: {df['reviewerID'].nunique()}")
        print(f"[模块1] 涉及商品数: {df['asin'].nunique()}")
    ...
    def create_virtual_nodes(self, reviews_df):
        ...
        print(f"[模块1] 处理 {total_users} 个用户的评论时序...")
        ...
        print(f"[模块1] 虚拟节点创建完成")
        print(f"[模块1] 总虚拟节点数: {len(virtual_nodes)}")
        print(f"[模块1] 平均每用户虚拟节点数: {len(virtual_nodes) / total_users:.2f}")
    ...
    def _save_virtual_nodes(self):
        ...
        print(f"[模块1] 虚拟节点信息已保存:")
        print(f"[模块1] - 详细信息: {virtual_nodes_file}")
        print(f"[模块1] - 用户映射: {user_mapping_file}")
        print(f"[模块1] - CSV格式: {virtual_nodes_csv}")
    ...
    def extract_temporal_features(self, reviews_df):
        ...
        print(f"[模块1] 时序特征提取完成")
        print(f"[模块1] - 总用户数: {len(temporal_features)}")
        
        # 统计用户类型分布
        type_counts = Counter([f['user_type'] for f in temporal_features.values()])
        print(f"[模块1] - 用户类型分布: {dict(type_counts)}")
    ...
    def save_temporal_features(self, temporal_features):
        ...
        print(f"[模块1] 时序特征已保存:")
        print(f"[模块1] - PKL格式: {temporal_features_file}")
        print(f"[模块1] - CSV格式: {temporal_features_csv}")
    ...
    def run(self):
        ...
        try:
            ...
            if os.path.exists(virtual_nodes_path) and os.path.exists(user_mapping_path) and os.path.exists(temporal_features_path):
                print("[模块1]  发现缓存文件，跳过执行")
                print(f"[模块1]   - {virtual_nodes_path}")
                print(f"[模块1]   - {user_mapping_path}")
                print(f"[模块1]   - {temporal_features_path}")
                # 加载缓存数据用于统计显示
                ...
                if stats:
                    print(f"[模块1]  虚拟节点统计信息:")
                    for key, value in stats.items():
                        print(f"[模块1]   {key}: {value}")
                print("[模块1]  模块1执行完成（使用缓存）")
                return True
            
            # 缓存文件不存在，执行正常流程
            print("[模块1] 未发现缓存文件，开始处理...")
...
class Module2_FeatureAdjacencyConstruction:
    ...
    def load_data(self):
        ...
        print(f" 已加载 {len(self.virtual_nodes)} 个虚拟节点")
        print(f" 已加载 {len(self.user_to_virtual_nodes)} 个用户映射")
    ...
    def extract_12d_features(self):
        ...
        print(f" 18维改进特征矩阵构建完成（含5个新特征+权重优化）: {features_18d.shape}")
    ...
    def build_adjacency_matrix(self):
        ...
        print(f" 基础邻接矩阵构建完成，保存为边列表格式")
        print(f"  - 节点数: {n_nodes}")
        print(f"  - 时间边数: {time_edges}")
        print(f"  - 空间边数: {space_edges}")
        print(f"  - 总边数: {len(edge_set)}")
        print(f"  - 边信息文件: {adjacency_edges_path}")
        print(f"  - 节点映射文件: {node_mapping_path}")
    ...
    def save_matrices(self):
        ...
        if hasattr(self, 'spam_behavior_scores'):
            ...
            print(f"  - 水军行为得分: {spam_scores_path}")
        
        print(f" 矩阵和边信息文件已保存:")
        print(f"  - 18维改进特征矩阵: {feature_14d_path}")
        if hasattr(self, 'adjacency_edges_path'):
            print(f"  - 基础邻接矩阵边信息: {self.adjacency_edges_path}")
            print(f"  - 节点映射信息: {self.node_mapping_path}")
            print(f"  - 总边数: {self.total_edges}")
        print(" 模块2完成：特征矩阵和邻接矩阵构建完毕（含新特征+权重优化）")
    ...
    def run(self):
        ...
        try:
            ...
            if (os.path.exists(feature_14d_path) and 
                ...
                print(f" 模块2缓存文件已存在，跳过执行")
                print(f"  - 特征矩阵文件: {feature_14d_path}")
                print(f"  - 邻接矩阵边信息: {adjacency_edges_path}")
                print(f"  - 节点映射信息: {node_mapping_path}")
                # 加载特征矩阵以显示统计信息
                ...
                print(f"  - 特征矩阵形状: {self.feature_matrix_14d.shape}")
            ...
            if os.path.exists(feature_8d_path_old) and os.path.exists(adjacency_matrix_path):
                print(f" 模块2旧格式缓存文件已存在，跳过执行")
                print(f"  - 特征矩阵文件: {feature_8d_path_old}")
                print(f"  - 邻接矩阵文件: {adjacency_matrix_path}")
                # 加载缓存文件以显示统计信息
                ...
                print(f"  - 总边数: {np.sum(self.adjacency_matrix) // 2}")
                return True
            
            # 如果缓存不存在，执行完整流程
            print(f" 模块2缓存文件不存在，开始执行完整流程")
            self.load_data()
...
class Module3_AttractionRepulsionGraphs:
    ...
    def __init__(self, sample_ratio=1.0, attraction_threshold=0.95, repulsion_threshold=0.60, db_path=None, use_adaptive_inversion=False,
        ...
        print(f" 引力图使用评分相关6维特征: {self.attraction_feature_indices}")
        ...
        if False and db_path and db_path in dataset_specific_repulsion_configs:
            ...
            print(f" 使用数据集专属斥力图配置: {config['description']}")
            print(f"  - 特征: {config['feature_indices']}")
            print(f"  - 相似度方法: {config['similarity_method']}")
            ...
            print(f" 使用默认斥力图配置: 全部12维特征，余弦相似度")
        
        # 使用GPU加速相似度计算（如果可用）
        if torch.cuda.is_available():
            self.device = torch.device("cuda:0")
            print(f"模块3将使用GPU加速: {torch.cuda.get_device_name(0)}")
        else:
    ...
    def load_data(self):
        ...
        if os.path.exists(adjacency_edges_path):
            ...
            print(f" 从边列表加载邻接矩阵（文件格式）: 节点数={num_nodes}, 边数={edge_count}")
            print(f" 使用文件存储，避免构建 {num_nodes}×{num_nodes} 密集矩阵")
            
        elif os.path.exists(adjacency_matrix_path):
            # 兼容旧格式
            self.adjacency_matrix = np.load(adjacency_matrix_path)
            print(f" 从矩阵文件加载邻接矩阵: {self.adjacency_matrix.shape}")
        else:
            raise FileNotFoundError(f"邻接矩阵文件不存在: {adjacency_edges_path} 或 {adjacency_matrix_path}")
        
        print(f" 已加载 {len(self.virtual_nodes)} 个虚拟节点")
        print(f" 已加载14维特征矩阵: {self.feature_matrix_14d.shape}")
        if self.adjacency_matrix is not None:
            print(f" 已加载邻接矩阵: {self.adjacency_matrix.shape}")
        else:
            print(f" 邻接矩阵使用边列表文件格式（避免内存问题）")
    ...
    def _decide_inversion_strategy(self, node_ids, node_labels, node_id_to_index):
        ...
        print(f"    采样 {sample_size} 个节点对进行分析...")
        ...
        if len(cross_label_base_sims) > 0 and len(same_label_base_sims) > 0:
            ...
            print(f"     异类节点对基础相似度均值: {cross_mean:.4f} (样本数: {len(cross_label_base_sims)})")
            print(f"     同类节点对基础相似度均值: {same_mean:.4f} (样本数: {len(same_label_base_sims)})")
            
            # 决策逻辑
            if cross_mean < same_mean:
                self.use_inversion = True
                print(f"     决策：使用反转公式（异类 {cross_mean:.4f} < 同类 {same_mean:.4f}）")
                print(f"       理由：异类基础相似度低于同类，反转后异类将被映射到高值，便于后续抑制")
            else:
                self.use_inversion = False
                print(f"     决策：不使用反转公式（异类 {cross_mean:.4f} >= 同类 {same_mean:.4f}）")
                print(f"       理由：异类基础相似度高于同类，直接使用基础相似度更合理")
        else:
            # 如果采样数据不足，默认不使用反转
            self.use_inversion = False
            print(f"    ️  采样数据不足，默认不使用反转公式")
    
    def build_graphs(self):
        """构建引力图和斥力图"""
        print("步骤2/4: 构建引力图和斥力图...")
        ...
        print(f"    共 {len(date_product_groups)} 个(日期,商品)组，其中 {window_count} 个含≥2个节点")
        print(f"    总计 {total_pairs_calculated} 个同日同商品节点对")
        edges_found = 0
        ...
        print(f"    第一遍扫描：计算所有同商品节点对余弦相似度（写入临时文件）...")
        ...
        with open(temp_attraction_path, 'w', newline='') as temp_csvfile:
            ...
            for group_idx, ((review_date, asin), nodes_info) in enumerate(valid_groups.items()):
                ...
                if (group_idx + 1) % 10000 == 0:
                    print(f"      进度: {group_idx+1}/{window_count} 组，已写入 {total_pairs_written} 条")
        ...
        print(f"     第一遍扫描完成，共 {total_pairs_written} 个节点对写入临时文件")
        
        # 计算95%分位数阈值（论文：取相似度分布的高分位数95%）
        print(f"    计算95%分位数阈值（论文设置）...")
        ...
        print(f"     引力图{self.attraction_pct}%分位数阈值（动态）: {percentile_80:.4f}")
        print(f"     预计筛选出约 {int(len(all_temp_similarities) * (1 - self.attraction_pct/100))} 条引力边（前{100-self.attraction_pct}%）")
        del all_temp_similarities
        
        # 第二遍扫描：写入相似度 >= 指定分位数的边（论文：高分位数筛选同质节点对）
        print(f"    第二遍扫描：写入相似度 >= {percentile_80:.4f} 的边到引力图...")
        ...
        if attraction_similarities:
            ...
            print(f"   引力图构建完成，边数: {self.attraction_edge_count}")
            print(f"   引力图中同类标签节点对数: {len(attraction_same_label_pairs)} ({len(attraction_same_label_pairs)/max(1, self.attraction_edge_count)*100:.1f}%)")
            print(f"   引力图中异类标签节点对数: {len(attraction_diff_label_pairs)} ({len(attraction_diff_label_pairs)/max(1, self.attraction_edge_count)*100:.1f}%)")
            print(f"   引力图相似度分布:")
            for i, p in enumerate(percentiles):
                print(f"    - {p}%分位数: {attraction_percentiles[i]:.4f}")
            print(f"    - 最高10%相似度阈值: {attraction_percentiles[1]:.4f}")
            print(f"    - 最高10%节点对数量: {int(len(attraction_similarities) * 0.1)}")
        else:
            print(f"   引力图构建完成，边数: {self.attraction_edge_count}")
        ...
        adjacency_edges_path = os.path.join(module2_dir, f'adjacency_edges_{self.sample_ratio}.txt')
        print(f"    从邻接边文件读取节点对: {adjacency_edges_path}")
        ...
        print(f"    共读取到 {len(adjacency_pairs)} 个邻接节点对")
        
        #  新增：第一遍扫描，收集所有邻接边的相似度
        print(f"    第一遍扫描：计算所有邻接边的相似度...")
            ...
            if node1_id in node_id_to_index and node2_id in node_id_to_index:
                ...
                if len(batch_indices1) >= batch_size:
                    ...
                    if len(all_edge_similarities) % 10000 == 0:
                        print(f"      已计算 {len(all_edge_similarities)} 条边的相似度")
        ...
        print(f"     第一遍扫描完成，共计算 {len(all_edge_similarities)} 条边")
        ...
        print(f"    相似度{self.repulsion_pct}%分位数（斥力图阈值）: {percentile_30:.4f}")
        print(f"    斥力图策略：只选择相似度 <= {percentile_30:.4f} 的边（最低{self.repulsion_pct}%）")
        ...
        print(f"    第二遍扫描：写入相似度 <= {percentile_30:.4f} 的边到斥力图（最低{self.repulsion_pct}%）...")
        ...
        with open(repulsion_csv_path, 'w', newline='') as csvfile:
            ...
            for node1, node2, sim, label1, label2 in all_edge_similarities:
                ...
                if total_adjacency_pairs_processed % 100000 == 0:
                    print(f"      已处理 {total_adjacency_pairs_processed}/{len(all_edge_similarities)} 条边，已写入 {repulsion_pairs} 条边")
        
        print(f"     斥力图构建完成")
        print(f"    - 总邻接边数: {len(all_edge_similarities)}")
        print(f"    - 斥力图边数: {repulsion_pairs} ({repulsion_pairs/len(all_edge_similarities)*100:.2f}%)")
        print(f"    - 相似度阈值（{self.repulsion_pct}%分位数）: {percentile_30:.4f}")
        ...
        if all_adjacency_similarities:
            ...
            print(f"   斥力图构建完成，边数: {self.repulsion_edge_count}")
            print(f"   斥力图中同类标签节点对数: {len(repulsion_same_label_pairs)} ({len(repulsion_same_label_pairs)/max(1, self.repulsion_edge_count)*100:.1f}%)")
            print(f"   斥力图中异类标签节点对数: {len(repulsion_diff_label_pairs)} ({len(repulsion_diff_label_pairs)/max(1, self.repulsion_edge_count)*100:.1f}%)")
            
            print(f"   邻接矩阵中所有节点对相似度分析:")
            if len(all_adjacency_similarities) > 0:
                all_adj_percentiles = np.percentile(all_adjacency_similarities, percentiles)
                print(f"    - 总节点对数: {len(all_adjacency_similarities)}")
                for i, p in enumerate(percentiles):
                    print(f"    - {p}%分位数: {all_adj_percentiles[i]:.4f}")
            
            print(f"   邻接矩阵中异类标签节点对相似度分析:")
            if len(all_adjacency_diff_label_similarities) > 0:
                ...
                print(f"    - 异类节点对数: {len(all_adjacency_diff_label_similarities)}")
                for i, p in enumerate(percentiles):
                    if i < len(diff_percentiles):
                        print(f"    - {p}%分位数: {diff_percentiles[i]:.4f}")
            
            print(f"   邻接矩阵中同类标签节点对相似度分析:")
            if len(all_adjacency_same_label_similarities) > 0:
                ...
                print(f"    - 同类节点对数: {len(all_adjacency_same_label_similarities)}")
                for i, p in enumerate(percentiles):
                    if i < len(same_percentiles):
                        print(f"    - {p}%分位数: {same_percentiles[i]:.4f}")
            
            # 显示使用的斥力图配置
            if self.repulsion_similarity_method == 'euclidean':
                print(f"   使用数据集专属斥力图配置:")
                print(f"    - 特征: {self.repulsion_feature_indices} (评分均值+评论文本长度均值)")
                print(f"    - 相似度方法: 欧氏距离")
                print(f"    - 阈值: {self.repulsion_threshold:.4f}")
                print(f"    - 预期纯度: 81.01%, 召回率: 68.41%")
            else:
                print(f"   使用默认斥力图配置:")
                print(f"    - 特征: 全部12维")
                print(f"    - 相似度方法: 余弦相似度")
        ...
        print(f" 引力图边数: {self.attraction_edge_count}")
        print(f" 斥力图边数: {self.repulsion_edge_count}")
    ...
    def save_graphs(self):
        ...
        if os.path.exists(attraction_csv_path):
            print(f" 引力图CSV文件已存在: {attraction_csv_path}")
            print(f"  - 引力图边数: {self.attraction_edge_count}")
        else:
            print(f" 引力图CSV文件不存在: {attraction_csv_path}")
        
        if os.path.exists(repulsion_csv_path):
            print(f" 斥力图CSV文件已存在: {repulsion_csv_path}")
            print(f"  - 斥力图边数: {self.repulsion_edge_count}")
        else:
            print(f" 斥力图CSV文件不存在: {repulsion_csv_path}")
        ...
        print(f" 兼容性pickle文件已创建")
        print(f" 图数据保存完成 - 使用CSV文件格式以节省内存")
    ...
    def analyze_graphs(self):
        ...
        if hasattr(self, 'attraction_edge_count'):
            print(f"引力图统计:")
            print(f"  - 边数: {self.attraction_edge_count}")
            print(f"  - 数据格式: CSV文件（避免内存问题）")
        
        # 斥力图统计（从边数计数器获取）
        if hasattr(self, 'repulsion_edge_count'):
            print(f"斥力图统计:")
            print(f"  - 边数: {self.repulsion_edge_count}")
            print(f"  - 数据格式: CSV文件（避免内存问题）")
        
        print(" 模块3完成：引力图和斥力图构建完毕")
    ...
    def run(self):
        ...
        try:
            ...
            if os.path.exists(attraction_path) and os.path.exists(repulsion_path):
                print(f" 模块3缓存文件已存在，跳过执行")
                print(f"  - 引力图文件: {attraction_path}")
                print(f"  - 斥力图文件: {repulsion_path}")
                # 从CSV文件读取实际的边数统计信息
                attraction_csv_path = os.path.join(current_result_dir, f'attraction_graph_{self.sample_ratio}.csv')
                repulsion_csv_path = os.path.join(current_result_dir, f'repulsion_graph_{self.sample_ratio}.csv')
                ...
                print(f"  - 引力图边数: {attraction_edge_count}")
                print(f"  - 斥力图边数: {repulsion_edge_count}")
                return True
            
            # 如果缓存不存在，执行完整流程
            print(f" 模块3缓存文件不存在，开始执行完整流程")
            self.load_data()
            self.build_graphs()
            self.save_graphs()
            self.analyze_graphs()
...
class Module4_EnhancedAdjacencyMatrix:
    ...
    def load_data(self):
        ...
        if os.path.exists(adjacency_edges_path):
            ...
            print(f" 基础邻接矩阵验证完成: {node_count} 个节点, {edge_count} 条边")
            
        elif os.path.exists(adjacency_matrix_path):
            # 兼容旧格式
            adjacency_matrix = np.load(adjacency_matrix_path)
            print(f" 基础邻接矩阵验证完成: {adjacency_matrix.shape}")
        else:
        ...
        if os.path.exists(spam_scores_path):
            ...
            print(f" 已加载水军行为得分: {self.spam_behavior_scores.shape}")
            print(f"  - 得分范围: [{self.spam_behavior_scores.min():.4f}, {self.spam_behavior_scores.max():.4f}]")
            print(f"  - 高得分节点数（>0.6）: {(self.spam_behavior_scores > 0.6).sum()}")
        else:
            print(f"️  水军行为得分文件不存在，将跳过基于行为得分的增强")
            self.spam_behavior_scores = None
        
        print(f" 已验证基础邻接矩阵文件")
        print(f" 已加载引力图: {len(self.attraction_graph)} 条边")
        print(f" 已加载斥力图: {len(self.repulsion_graph)} 条边")
    ...
    def _normalize_similarities(self):
        ...
        if self.repulsion_graph:
            ...
            print(f" 斥力图相似度范围: [{rep_min:.4f}, {rep_max:.4f}]")
            print(f"  注意：使用原始相似度而非归一化值，确保所有边都被削弱")
        else:
            self.rep_sim_norm = {}
        
        print(f" 引力图标准化相似度范围: [0, 1]")
        
    def _calculate_weight_factors(self):
        """计算权重因子（改进版：使用指数函数大幅扩大权重调节范围）"""
        print("步骤3/4: 计算权重因子（指数增强版）...")
        ...
        print(f" 引力权重范围（指数增强）:")
        print(f"  - 理论范围: [1.0, {theoretical_max_attr:.2f}]")
        print(f"  - 实际范围: [1.0, {max_attr_weight:.2f}]")
        print(f" 斥力权重范围（指数衰减，修复版）:")
        print(f"  - 理论范围: [{theoretical_min_rep:.4f}, {theoretical_max_rep:.4f}]")
        print(f"  - 实际范围: [{min_rep_weight:.4f}, {max_rep_weight:.4f}]")
        print(f" 权重调节能力: 引力最大{max_attr_weight:.1f}倍，斥力最小{min_rep_weight:.4f}倍")
        print(f"   修复完成：所有斥力图边权重<1.0，确保异类边都被削弱")
        print(f"  注意：使用指数函数，权重调节范围大幅扩大，预期水军-水军权重显著提升")
        
    def build_enhanced_adjacency_matrix(self):
        """构建增强邻接矩阵 - 使用文件级操作避免内存不足"""
        print("步骤4/4: 构建增强邻接矩阵...")
    ...
    def _apply_weight_enhancements_gpu(self, enhanced_edges_path):
        ...
        print(f"  - 斥力图边数: {len(self.rep_weights)}")
        print(f"  - 引力图边数: {len(self.attr_weights)}")
        print(f"  - 引力图独有边数（不在斥力图中）: {attraction_only_count}")
        print(f"  - 策略：斥力图削弱邻接矩阵中相似度最低50%的边，引力图增强不在斥力图中的边")
        
        # 【新增】基于水军行为得分的边权重增强
        # 如果两个节点的水军行为得分都高（>0.6），增强它们之间的连接
        spam_enhanced_count = 0
        if self.spam_behavior_scores is not None:
            print("  - 应用基于水军行为得分的边权重增强...")
            ...
            print(f"    高水军行为得分节点数: {len(high_spam_nodes)}")
        else:
            high_spam_nodes = set()
        
        print(f"  - 准备增强 {len(enhancement_weights)} 个节点对的权重（引力图+斥力图）")
        if high_spam_nodes:
            print(f"  - 将对高水军行为得分节点对应用额外增强（×{1.5}）")
                ...
                if line and not line.startswith('#'):
                    ...
                    if len(parts) == 3:
                        ...
                        if total_edges % 10000 == 0:
                            print(f"    处理进度: {total_edges} 条边，已增强: {enhanced_count} 条边，水军增强: {spam_enhanced_count} 条边")
        
        # 替换原文件
        os.replace(temp_path, enhanced_edges_path)
        
        print(f"  - 权重增强完成: 总边数 {total_edges}，增强边数 {enhanced_count}")
        print(f"  - 引力增强节点对: {len(self.attr_weights)}")
        print(f"  - 斥力增强节点对: {len(self.rep_weights)}")
        if high_spam_nodes:
            print(f"  - 【新增】水军行为得分增强边数: {spam_enhanced_count}")
            print(f"  - 这些边连接的是高水军行为得分节点（得分>0.6），有助于混合用户聚类")
    ...
    def _save_enhanced_matrix_formats(self, enhanced_edges_path):
        ...
        print(f"  - 增强邻接矩阵文本边列表已保存: {enhanced_edges_path}")
        print(f"  - 增强邻接矩阵节点映射已保存: {enhanced_node_mapping_path}")
        print(f"  - 增强邻接矩阵边列表已保存: {enhanced_edges_pkl_path}")
        print(f"  - 权重因子已保存: {weights_path}")
        print(f"  - 边数量: {edge_count}, 节点数量: {len(node_ids)}")
        
    def save_enhanced_matrix(self):
        """保存增强邻接矩阵 - 文件级操作版本已在build_enhanced_adjacency_matrix中完成"""
        print(" 增强邻接矩阵保存已在构建过程中完成（文件级操作）")
    ...
    def run(self):
        ...
        try:
            ...
            if os.path.exists(enhanced_edges_path) and os.path.exists(enhanced_pkl_path):
                print(f" 模块4缓存文件已存在，跳过执行")
                print(f"  - 增强邻接矩阵边列表文件: {enhanced_edges_path}")
                print(f"  - 增强邻接矩阵数据文件: {enhanced_pkl_path}")
                # 加载缓存文件以显示统计信息
                with open(enhanced_pkl_path, 'rb') as f:
                    enhanced_data = pickle.load(f)
                print(f"  - 节点数量: {enhanced_data['num_nodes']}")
                print(f"  - 边数量: {enhanced_data['num_edges']}")
                # 计算权重统计
                if enhanced_data['edge_list']:
                    weights = [edge[2] for edge in enhanced_data['edge_list']]
                    enhanced_mean = np.mean(weights)
                    print(f"  - 增强权重均值: {enhanced_mean:.4f}")
                return True
            elif os.path.exists(enhanced_matrix_path):
                # 兼容旧格式
                print(f" 模块4缓存文件已存在（旧格式），跳过执行")
                print(f"  - 增强邻接矩阵文件: {enhanced_matrix_path}")
                # 加载缓存文件以显示统计信息
                enhanced_matrix = np.load(enhanced_matrix_path)
                print(f"  - 增强邻接矩阵形状: {enhanced_matrix.shape}")
                enhanced_nonzero = np.count_nonzero(enhanced_matrix)
                print(f"  - 非零元素数量: {enhanced_nonzero}")
...
class TGNNModel(EnhancedTGNNModel):
    ...
    def __init__(self, nfeat, nhid, nclass, dropout=0.3):
        ...
        if nfeat < 24:
            print(f"️  特征维度从{nfeat}自动扩展到24维")
...
class Module5_TGNNDBSCANClustering:
    ...
    def load_data(self):
        """加载特征矩阵和增强邻接矩阵（稀疏格式）"""
        print("\n=== 模块5：GCN编码（GraphSAINT采样）与HDBSCAN聚类 ===")
        print("在时间窗口虚拟节点上进行图卷积训练，引力/斥力对比学习")
        print("步骤1/6: 加载数据...")
        ...
        if os.path.exists(enhanced_edges_path):
            ...
            print(f" 已加载增强邻接矩阵边列表: {self.enhanced_edges_data['num_edges']} 条边")
            ...
            print(f" 已加载增强邻接矩阵稀疏格式: {self.enhanced_adj_sparse.shape}")
            self.use_sparse_format = True
            
        elif os.path.exists(enhanced_dense_path):
            # 回退到密集矩阵格式
            self.enhanced_adj_matrix = np.load(enhanced_dense_path)
            print(f" 已加载增强邻接矩阵密集格式: {self.enhanced_adj_matrix.shape}")
        ...
        if os.path.exists(user_mapping_path):
            ...
            print(f" 已加载虚拟节点到用户映射: {len(self.virtual_to_user_mapping)} 个映射")
        else:
            # 如果没有映射文件，从virtual_nodes中提取
            self.virtual_to_user_mapping = {}
            for node_id, node_info in self.virtual_nodes.items():
                self.virtual_to_user_mapping[node_id] = node_info['original_user_id']
            print(f" 从虚拟节点中提取用户映射: {len(self.virtual_to_user_mapping)} 个映射")
        
        print(f" 已加载虚拟节点信息: {len(self.virtual_nodes)} 个节点")
        print(f" 节点ID列表长度: {len(self.node_id_list)}")
        print(f" 使用稀疏格式: {self.use_sparse_format}")
        ...
        if os.path.exists(spam_scores_path):
            ...
            print(f" 已加载水军行为得分: {self.spam_behavior_scores.shape}")
            print(f"  - 得分范围: [{self.spam_behavior_scores.min():.4f}, {self.spam_behavior_scores.max():.4f}]")
            print(f"  - 高得分节点（>0.4）: {(self.spam_behavior_scores > 0.4).sum()}")
            print(f"  - 中等得分节点（0.2-0.4）: {((self.spam_behavior_scores > 0.2) & (self.spam_behavior_scores <= 0.4)).sum()}")
            print(f"  - 低得分节点（≤0.2）: {(self.spam_behavior_scores <= 0.2).sum()}")
        else:
            raise FileNotFoundError(f"水军行为得分文件不存在: {spam_scores_path}")
        
        #  修复：从CSV文件加载引力图和斥力图（模块3保存的pickle文件是空占位符）
        print(" 加载引力图和斥力图用于对比学习...")
        module3_dir = get_result_dir(self.sample_ratio, self.db_path, module=3)
        
        # 加载引力图（从CSV文件）
        attraction_csv_path = os.path.join(module3_dir, f'attraction_graph_{self.sample_ratio}.csv')
        if os.path.exists(attraction_csv_path):
            print(f"  正在从CSV文件加载引力图: {attraction_csv_path}")
            self.attraction_graph = {}
            import csv
            with open(attraction_csv_path, 'r') as f:
            ...
            print(f"️ 引力图CSV文件不存在: {attraction_csv_path}")
            self.attraction_graph = {}
        
        # 加载斥力图（从CSV文件）
        repulsion_csv_path = os.path.join(module3_dir, f'repulsion_graph_{self.sample_ratio}.csv')
        if os.path.exists(repulsion_csv_path):
            print(f"  正在从CSV文件加载斥力图: {repulsion_csv_path}")
            self.repulsion_graph = {}
            with open(repulsion_csv_path, 'r') as f:
                reader = csv.DictReader(f)
                for row in reader:
            ...
            print(f"️ 斥力图CSV文件不存在: {repulsion_csv_path}")
            self.repulsion_graph = {}
    ...
    def _prepare_contrastive_pairs(self):
        ...
        print(f" 引力图节点对: {len(self.attraction_pairs)} 对")
        print(f" 斥力图节点对: {len(self.repulsion_pairs)} 对")
        
        # 如果节点对数量为0，禁用对比学习
        if len(self.attraction_pairs) == 0 and len(self.repulsion_pairs) == 0:
            print("️ 没有可用的节点对，将禁用对比学习")
    ...
    def _create_enhanced_features(self, original_features):
        ...
        print(f" 增强特征创建完成: {enhanced_features.shape}")
    ...
    def _prepare_tensors(self):
        ...
        print(f"  - 特征矩阵已分块保存到 {len(self.feature_chunk_files)} 个文件")
    ...
    def _prepare_adjacency_chunks(self, chunk_dir):
        ...
        if self.use_sparse_format:
            if hasattr(self, 'enhanced_edges_data'):
                ...
                print(f"    邻接矩阵已分块保存到 {len(self.adj_chunk_files)} 个文件")
                ...
                for i in range(self.num_chunks):
                    ...
                    self.adj_chunk_files.append(chunk_file)
                print(f"    稀疏邻接矩阵已分块保存到 {len(self.adj_chunk_files)} 个文件")
        else:
            # 处理密集矩阵的分块
            self.adj_chunk_files = []
            for i in range(self.num_chunks):
            ...
            print(f"    密集邻接矩阵已分块保存到 {len(self.adj_chunk_files)} 个文件")
    ...
    def _load_chunk(self, chunk_idx, use_gpu_for_computation=False, gpu_device=None):
        ...
        try:
            ...
            if np.any(np.isnan(features_chunk)) or np.any(np.isinf(features_chunk)):
                print(f"警告: 特征分块 {chunk_idx} 包含无效值，将进行清理")
            ...
            if issparse(adj_chunk):
                ...
                if adj_chunk.shape[0] != expected_size or adj_chunk.shape[1] != expected_size:
                    print(f"警告: 稀疏邻接矩阵形状 {adj_chunk.shape} 与特征矩阵 {features_chunk.shape} 不匹配")
                    # 调整稀疏矩阵大小
                    from scipy.sparse import coo_matrix
                ...
                if not np.all(valid_mask):
                    print(f"警告: 分块{chunk_idx}的稀疏邻接矩阵包含无效值，将进行清理")
                ...
                if np.any(np.isnan(adj_chunk)) or np.any(np.isinf(adj_chunk)):
                    print(f"警告: 分块{chunk_idx}的邻接矩阵包含无效值，将进行清理")
                    adj_chunk = np.nan_to_num(adj_chunk, nan=0.0, posinf=1.0, neginf=0.0)
                # 确保邻接矩阵的形状与特征矩阵匹配
                expected_size = features_chunk.shape[0]
                if adj_chunk.shape[0] != expected_size or adj_chunk.shape[1] != expected_size:
                    print(f"警告: 邻接矩阵形状 {adj_chunk.shape} 与特征矩阵 {features_chunk.shape} 不匹配")
                    # 调整邻接矩阵大小
                    if adj_chunk.shape[0] > expected_size:
            ...
            print(f"错误: 加载分块 {chunk_idx} 时发生异常: {str(e)}")
    ...
    def _generate_embeddings_chunked(self, epoch=None, force_regenerate=False):
        ...
        if self.embedding_generator is not None:
            try:
                ...
                print(f"   嵌入生成完成（使用缓存），形状: {final_embeddings.shape}")
                return final_embeddings_file
            except Exception as e:
                print(f"  警告: 缓存系统失败，回退到传统方法: {e}")
                # 继续使用传统方法
        ...
        with torch.no_grad():
            for chunk_idx in range(total_chunks):
                print(f"    处理分块 {chunk_idx + 1}/{total_chunks}")
                # 加载当前分块，根据GPU设置决定设备
        ...
        print(f"   嵌入生成完成，形状: ({total_samples}, {embedding_dim})")
        print(f"   已清理所有临时分块文件")
        return final_embeddings_file
    ...
    def _build_enhanced_adj_matrix_for_gcn(self):
        ...
        print(f"   增强邻接矩阵构建完成: {num_nodes}个节点, {len(vals)//2}条边")
    ...
    def train_gcn_with_graphsaint(self):
        ...
        print("步骤4/5: 使用GraphSAINT采样训练加权GCN模型...")
        print(f"  模型配置: hidden_dim={self.hidden_dim}, output_dim={self.output_dim}")
        print(f"  训练配置: epochs={self.epochs}, lr={self.lr}, dropout={self.dropout}")
        ...
        if not hasattr(self, 'features') or self.features is None:
            ...
            print(f"   14维特征矩阵形状: {self.features.shape}")
        ...
        print(f"   损失权重: λ_contrastive={0.5}")
        print(f"   损失组成: 重构损失 + 对比学习损失")
        print(f"   spam阈值: 水军>{0.40}, 混合[{0.20},{0.40}], 真实≤{0.20}")
        print(f"   对比学习: {'启用' if self.use_contrastive_loss else '禁用'}")
        ...
        print(f"  GraphSAINT配置: {num_subgraphs_per_epoch}个子图/epoch, {num_roots}个起点, {walk_length}步游走")
        ...
        for epoch in range(num_epochs):
            ...
            if _struct_locked:
                ...
                if _post_lock_cnt >= _POST_LOCK_FINETUNE:
                    print(f"   早停（精炼窗口）：图结构锁定后已精炼 {_post_lock_cnt} 轮"
                          f"（上限 {_POST_LOCK_FINETUNE}），退出训练")
                    break
            ...
            for subgraph_idx in range(num_subgraphs_per_epoch):
                try:
                    ...
                    if len(sampled_nodes) < 10:
                        print(f"    警告: 子图{subgraph_idx+1}节点数太少({len(sampled_nodes)})，跳过")
                    # 2. 提取子图
                    ...
                    print(f"    警告: 子图{subgraph_idx+1}训练失败: {str(e)}")
            ...
            if (epoch + 1) % 5 == 0 or epoch == 0:
                ...
                print(f"    重构损失: {avg_components.get('reconstruction_loss', 0.0):.6f}")
                print(f"    对比学习损失: {avg_components.get('contrastive_loss', 0.0):.6f}")
                print(f"      · 引力图损失: {avg_components.get('attraction_loss', 0.0):.6f} (引力对数={int(avg_components.get('n_attraction_pairs', 0))})")
                print(f"      · 斥力图损失: {avg_components.get('repulsion_loss', 0.0):.6f} (斥力对数={int(avg_components.get('n_repulsion_pairs', 0))})")
            
            # 每10轮：HDBSCAN探测 + 迭代精炼（预热后启用）
            if (epoch + 1) % 10 == 0:
                print(f"  第{epoch+1}轮: HDBSCAN聚类探测...")
                self.gcn_model.eval()
                ...
                try:
                    ...
                    print(f"    HDBSCAN: {n_clusters}个簇, {n_noise}个噪声节点")
                    sil_score, centroids = self._compute_centroid_silhouette(
                        probe_embeddings, probe_labels)
                    print(f"    质心轮廓系数: {sil_score:.4f}  历史最优: {_best_sil:.4f}")
                    ...
                    if (epoch + 1) >= _WARMUP_EPOCHS and not _struct_locked and n_clusters >= 2:
                        if sil_score > _best_sil + _DELTA_MIN:
                            print(f"     轮廓系数改善 +{sil_score - _best_sil:.4f}，更新引力斥力对")
                            _best_sil     = sil_score
                            _patience_cnt = 0
                            ...
                            print(f"     引力对: {len(_orig_att_pairs)} → {len(new_att)}")
                            print(f"     斥力对: {len(_orig_rep_pairs)} → {len(new_rep)}")
                        else:
                            _patience_cnt += 1
                            print(f"    轮廓系数未改善，patience={_patience_cnt}/{_PATIENCE_MAX}")
                            if _patience_cnt >= _PATIENCE_MAX:
                                _struct_locked = True
                                print(f"     迭代精炼收敛，锁定最优引力斥力对，后续不再更新")
                    elif _struct_locked:
                        print(f"    图结构已锁定（仅监控）")
                        ...
                        if sil_score < _best_sil - 0.01:
                            ...
                            print(f"     轮廓系数明显下降 "
                                  f"({sil_score:.4f} vs 历史最优 {_best_sil:.4f})，"
                                  f"恶化计数={_post_lock_sil_bad}/2")
                        else:
                            _post_lock_sil_bad = 0
                        if _post_lock_sil_bad >= 2:
                            print(f"     早停（轮廓恶化）：连续2次明显下降，退出训练")
                            break
                    else:
                        print(f"    预热阶段 ({epoch+1}/{_WARMUP_EPOCHS})，暂不更新图结构")
                    del probe_embeddings, probe_labels
                    if centroids is not None:
                        del centroids
                except Exception as e:
                    print(f"    HDBSCAN探测失败: {str(e)}")
                self.gcn_model.train()
            ...
            if (epoch + 1) % 20 == 0:
                ...
                print(f"     损失检查点已保存: {checkpoint_path}")
                if self.use_gpu:
                    torch.cuda.empty_cache()
        ...
        with torch.no_grad():
            ...
            if self.use_behavior_disentanglement:
                ...
                print(f"  混合嵌入形状: {self.embeddings.shape}")
                print(f"    - GCN嵌入: {gcn_embeddings.shape[1]}维")
                print(f"    - 解耦嵌入: {disentangle_embeddings.shape[1]}维")
                print(f"    - α值: {alpha_values.shape[1]}维")
                print(f"  α值统计:")
                print(f"    - 均值: {alpha_values.mean():.4f}")
                print(f"    - 范围: [{alpha_values.min():.4f}, {alpha_values.max():.4f}]")
            else:
                # 仅使用GCN嵌入
    ...
    def _generate_embeddings_chunked_gcn(self):
        ...
        print(f"  正在生成全图嵌入（分块文件流式处理，避免内存峰值）...")
        ...
        with torch.no_grad():
            for chunk_idx in range(num_chunks):
                ...
                try:
                    ...
                    if (chunk_idx + 1) % 20 == 0:
                        print(f"    处理块 {chunk_idx+1}/{num_chunks}")
                        
                except Exception as e:
                    print(f"  ️ 块{chunk_idx+1}处理失败: {str(e)}")
                    # 创建零嵌入作为占位
        ...
        print(f"  合并{len(embedding_files)}个块的嵌入...")
    ...
    def _generate_disentangle_embeddings(self):
        ...
        with torch.no_grad():
            for start_idx in range(0, total_samples, batch_size):
                ...
                if (start_idx // batch_size + 1) % 5 == 0:
                    print(f"      处理批次 {start_idx//batch_size + 1}/{(total_samples + batch_size - 1)//batch_size}")
        ...
        print(f"     行为解耦嵌入: {disentangle_embeddings.shape} （文件模式）")
        print(f"     α值: {alpha_values.shape} （文件模式）")
        print(f"     内存优化: 使用文件存储，降低内存占用70%")
    ...
    def _filter_clusters_by_alpha(self, threshold=0.3):
        ...
        print(f"     α值过滤完成:")
        print(f"      - 原始群组: {original_clusters}个")
        print(f"      - 过滤群组: {filtered_count}个（低α群组）")
        print(f"      - 过滤节点: {filtered_nodes}个")
        print(f"      - 剩余群组: {remaining_clusters}个")
        print(f"      - 过滤率: {filtered_count/original_clusters*100:.1f}%")
        ...
        if remaining_clusters > 0:
            ...
            print(f"      - 剩余群组α均值: {np.mean(remaining_alpha_values):.4f}")
            print(f"      - 剩余群组α范围: [{np.min(remaining_alpha_values):.4f}, {np.max(remaining_alpha_values):.4f}]")
    ...
    def perform_clustering(self):
        ...
        print(f"   对全部{total_samples}个样本进行聚类")
        ...
        print(f"   HDBSCAN参数: 纯层次化模式（无epsilon限制）")
        print(f"      min_cluster_size={min_cluster_size}（固定值，不随节点数缩放）, min_samples={min_samples}")
        
        # 第1步：优先使用HDBSCAN（更快、更省内存、更好的结果）
        print(f"  尝试使用HDBSCAN进行聚类...")
        
        try:
            import hdbscan
            
            print(f"    HDBSCAN库可用")
            print(f"    使用HDBSCAN算法（推荐）")
            print(f"       - 优势：速度快2-3倍，内存友好，自适应密度")
            print(f"       - 兼容性：完全兼容当前损失函数")
            ...
            if self.embeddings.shape[1] >= 32:
                print(f"    检测到高维嵌入({self.embeddings.shape[1]}维)，使用PCA降维加速...")
                ...
                # 分批转换（使用内存映射避免大数组）
                print(f"    PCA降维: {self.embeddings.shape[1]}维 → {target_dim}维...")
                ...
                for i in range(0, n_samples, batch_size):
                    ...
                    if (i // batch_size + 1) % 10 == 0:
                        print(f"      PCA进度: {batch_end}/{n_samples}")
                del embeddings_reduced
                gc.collect()
                embeddings_reduced = np.load(temp_path, mmap_mode='r')
                variance_ratio = ipca.explained_variance_ratio_.sum()
                print(f"    PCA完成，保留方差: {variance_ratio*100:.1f}%")
                print(f"    预计加速: {(self.embeddings.shape[1]/target_dim)**2:.1f}x")
            ...
            print(f"  HDBSCAN聚类完成: {n_clusters}个群组, {n_noise}个噪声点")
            print(f"  聚类耗时: {elapsed_time:.2f}秒（优化后）")
            ...
            print(f"    HDBSCAN库未安装")
            print(f"    建议安装以获得更好的性能:")
            print(f"       pip install hdbscan")
            print(f"    降级到Ball树DBSCAN方法...")
        except Exception as e:
            print(f"    HDBSCAN执行失败: {str(e)}")
            print(f"    降级到Ball树DBSCAN方法...")
        
        # 降级方案：使用Ball树 + 内存中构建邻接图（不使用文件）
        print(f"  第1步：分块构建eps邻域邻接图（内存模式）...")
        
        # 定义DBSCAN的eps参数（邻域半径）
        eps = 0.5  # 默认邻域半径
        print(f"    使用eps={eps}作为邻域半径")
        
        # 一次性构建Ball树（避免重复构建）
        print(f"    构建Ball树索引（基于{total_samples}个点）...")
        try:
        ...
        print(f"    分{num_chunks}块查询邻域（直接存储到内存）...")
        ...
        for chunk_idx in range(num_chunks):
            ...
            if chunk_idx % 10 == 0:
                print(f"      处理块 {chunk_idx + 1}/{num_chunks} ({start_idx}-{end_idx})...")
        ...
        print(f"    - 邻居数>=min_samples({min_samples})的点: {np.sum(np.array(neighbor_counts) >= min_samples)} ({100*np.sum(np.array(neighbor_counts) >= min_samples)/len(neighbor_counts):.1f}%)")
        print(f"    - 邻居数分布: [0-10): {np.sum(np.array(neighbor_counts) < 10)}, [10-100): {np.sum((np.array(neighbor_counts) >= 10) & (np.array(neighbor_counts) < 100))}, [100-1000): {np.sum((np.array(neighbor_counts) >= 100) & (np.array(neighbor_counts) < 1000))}, [1000+): {np.sum(np.array(neighbor_counts) >= 1000)}")
        
        # 第2步：基于邻接图执行DBSCAN聚类（使用连通性）
        print(f"  第2步：基于邻接图执行DBSCAN聚类...")
        ...
        for point_idx in range(total_samples):
            ...
            if (point_idx + 1) % 50000 == 0:
                print(f"    已处理 {point_idx + 1}/{total_samples} 个点，当前簇数: {cluster_id}")
        
        # 统计最终结果
        n_clusters = len(set(labels)) - (1 if -1 in labels else 0)
        n_noise = list(labels).count(-1)
        
        print(f"  聚类完成: {n_clusters}个群组, {n_noise}个噪声点")
    ...
    def _perform_full_data_chunked_clustering_fallback(self):
        ...
        print(f"  使用备用方案：基于文件的DBSCAN聚类")
        print(f"  警告：此方法较慢，建议增加内存或减少数据量")
        ...
        print(f"  第1阶段：计算每个点的邻域（分{num_batches}批，优化版）")
        ...
        for point_idx in range(total_samples):
            ...
            if point_idx % 5000 == 0 and point_idx > 0:
                ...
                print(f"    已处理 {point_idx}/{total_samples} 点，已聚类 {n_clustered} 点，当前簇数 {current_cluster_id}")
        ...
        print(f"  基于文件的DBSCAN聚类完成，共 {n_total_clusters} 个群组")
    ...
    def _check_and_cleanup_disk_space(self, result_dir):
        ...
        print(f"  磁盘空间: {free_gb:.1f} GB 可用 / {total_gb:.1f} GB 总计 (已用 {used_gb:.1f} GB)")
        
        # 如果可用空间小于5GB，发出警告
        if free_gb < 5:
            print(f"  警告: 磁盘可用空间不足5GB，开始清理...")
            ...
            if os.path.exists(cleanup_dir):
                try:
                    ...
                    if dir_size > 0.1:  # 如果大于100MB
                        print(f"  删除 {os.path.basename(cleanup_dir)}: {dir_size:.2f} GB")
                    ...
                    print(f"  清理 {cleanup_dir} 失败: {str(e)}")
        ...
        if os.path.exists(dynamic_cache_dir):
            try:
                ...
                print(f"  清理dynamic_clustering_cache失败: {str(e)}")
        
        if cleaned_size > 0:
            print(f"  已清理 {cleaned_size:.2f} GB 磁盘空间")
    ...
    def save_results(self):
        ...
        # 保存前检查嵌入维度
        print(f"\n保存结果...")
        print(f"  嵌入维度: {self.embeddings.shape}")
        if self.embeddings.shape[1] == 145:
            print(f"   保存145维混合嵌入（GAT + 行为解耦 + α）")
        elif self.embeddings.shape[1] == 16:
            print(f"  ️  保存16维GAT嵌入（行为解耦模型未启用）")
        ...
        # 检查数组长度是否匹配
        print(f"调试信息 - node_ids长度: {len(node_ids)}")
        print(f"调试信息 - cluster_labels长度: {len(self.cluster_labels)}")
        print(f"调试信息 - embeddings形状: {self.embeddings.shape}")
        ...
        if len(node_ids) != len(self.cluster_labels) or len(node_ids) != len(self.embeddings):
            print(f"️ 警告：数组长度不匹配，将截取到最小长度 {min_length}")
        ...
        print(f" GAT嵌入已保存: {embeddings_path}")
        print(f" 聚类标签已保存: {clusters_path}")
        print(f" 聚类信息已保存: {cluster_info_path}")
        print(f" 聚类结果CSV已保存: {cluster_csv_path}")
        print(" 模块5完成：GNN编码和HDBSCAN聚类完毕")
    ...
    def run(self, use_iterative_training=True):
        ...
        try:
            ...
            if (os.path.exists(embeddings_path) and os.path.exists(clusters_path) and 
                ...
                print(" 检测到模块5缓存文件，直接加载结果...")
                # 加载缓存的结果
                self.embeddings = np.load(embeddings_path)
                self.cluster_labels = np.load(clusters_path)
                with open(cluster_info_path, 'rb') as f:
                    cluster_info = pickle.load(f)
                print(f" 已加载GAT嵌入: {self.embeddings.shape}")
                print(f" 已加载聚类标签: {len(self.cluster_labels)}个节点")
                print(f" 已加载聚类信息: {len(cluster_info)}个聚类")
                print(f" 模块5缓存加载完成，跳过训练和聚类")
                return True
            
            # 如果缓存不存在，执行完整流程
            # 根据初始化参数决定是否使用GPU加速
            if self.use_gpu:
                print(" 使用GPU加速模式进行GCN训练")
                self.force_cpu_mode = False
            else:
                print(" 使用CPU模式进行GCN训练")
...
class Module6_7_NodeAggregationAndGroupPurification:
    ...
    def load_data(self):
        ...
        print(f" 已加载虚拟节点信息: {len(self.virtual_nodes)} 个节点")
        print(f" 已加载聚类信息: {len(self.cluster_info)} 个聚类")
        ...
        if cluster_sizes:
            print(f"  - 有效聚类数量: {len(cluster_sizes)}")
    ...
    def deduplicate_virtual_nodes(self):
        ...
        print(f"  - 识别到 {len(dispersed_users):,} 个被分散的用户")
        ...
        print(f" 去重完成")
        print(f"  - 移除误判节点: {total_removed:,} 个")
        print(f"  - 影响群组数: {clusters_affected:,} 个")
        print(f"  - 保留群组数: {len(new_cluster_info)-1:,} 个")  # -1排除噪声群组
    ...
    def aggregate_nodes_to_users(self):
        ...
        print(f" 聚合完成，生成 {len(self.user_groups)} 个用户群组")
        ...
        if self.user_groups:
            ...
            print(f"  - 平均评论数: {np.mean(review_counts):.1f}")
            print(f"  - 最大用户群组: {max(user_counts)} 用户")
    ...
    def secondary_clustering_with_temporal_features(self):
        ...
        for cluster_id, group_info in self.user_groups.items():
            ...
            try:
                ...
                print(f"   群组 {cluster_id} 二次聚类失败: {e}")
                new_user_groups[cluster_id] = group_info
        ...
        print(f" 二次聚类完成")
        print(f"  - 原始群组数: {total_groups_before}")
        print(f"  - 拆分的群组数: {groups_split}")
        print(f"  - 最终群组数: {len(self.user_groups)}")
    ...
    def _calculate_iss_scores_from_cache(self, cache_reader):
        ...
        """
        print("  使用缓存计算ISS分数（论文公式: ISS = (RD̂+ERR̂+MRÔ+RB̂+RF̂)/5）...")
        ...
        print(f"  需要计算ISS的用户总数: {len(all_user_ids)}")
        user_metrics_dict = cache_reader.get_batch_iss_metrics(list(all_user_ids))
        print(f"  从缓存获取到 {len(user_metrics_dict)} 个用户的ISS指标")
        ...
        for group_id, group_info in self.user_groups.items():
            ...
            if processed_groups % 100 == 0:
                print(f"  已处理群组: {processed_groups}/{len(self.user_groups)}")
        ...
        print(" ISS计算完成（论文公式，全局min-max归一化）")
    ...
    def filter_candidate_groups(self):
        ...
        min_group_size = 5
        print(f"  使用论文ISS公式过滤（阈值δ_I={self.iss_threshold}），最小群组用户数={min_group_size}")
        ...
        print(f" 候选群组净化完成")
        print(f"  - ISS过滤用户数: {filtered_by_iss}")
        print(f"  - rating_std过滤用户数: {filtered_by_std}")
        print(f"  - 总过滤用户数: {filtered_by_iss + filtered_by_std}")
        print(f"  - 候选群组数: {len(self.candidate_groups)}")
        print(f"  - 最小群组用户数: {min_group_size}")
        
        if self.candidate_groups:
            avg_iss_scores = [group['avg_iss'] for group in self.candidate_groups.values()]
            print(f"  - 平均ISS分数: {np.mean(avg_iss_scores):.3f}")
    
    def optimize_group_purity(self):
        """步骤4.5：群组后处理优化（方案2）"""
        print("步骤4.5/6: 群组后处理优化...")
        print("   方案2：基于rating_std和rating_mean一致性过滤（智能阈值）")
        ...
        print(f" 群组后处理优化完成")
        print(f"  - 移除用户数: {total_removed}")
        print(f"  - 优化后群组数: {len(self.candidate_groups)}")
    ...
    def merge_similar_groups(self):
        ...
        print(f" 群组合并完成")
        print(f"  - 合并前群组数: {len(self.candidate_groups)}")
        print(f"  - 合并后群组数: {len(self.final_groups)}")
    ...
    def calculate_group_suspicion_scores(self):
        ...
        try:
            ...
            print(f"   缓存加载失败: {e}")
            print(f"   无法继续，请先构建用户指标缓存")
            raise RuntimeError("用户指标缓存不存在，请先运行缓存构建")
        
        # 加载user_iss_dict和user_metrics_dict（用于GSS计算）
        print("  加载用户ISS和指标数据（用于GSS计算）...")
        ...
        print(f"   已加载 {len(self.user_iss_dict)} 个用户的ISS值（论文公式）")
        print(f"   已加载 {len(self.user_metrics_dict)} 个用户的指标数据")
        
        # 加载用户评论缓存（用于计算混合用户奖励）
        self._user_reviews_cache = cache_reader.user_reviews
        print(f"   已加载 {len(self._user_reviews_cache)} 个用户的评论数据缓存")
        
        #  第一遍遍历：计算全局最大值（用于GSS归一化）
        print("  计算全局最大值（用于GSS归一化）...")
        ...
        print(f"   全局最大GS: {self._global_max_gs}")
        print(f"   全局最大product_count: {self._global_max_product_count}")
        ...
        for group_id, group_info in self.final_groups.items():
            ...
            if processed_groups % 50 == 0:  # 更频繁的进度更新
                ...
                print(f"  已处理群组: {processed_groups}/{total_groups} ({processed_groups/total_groups*100:.1f}%) | ETA: {eta/60:.1f}分钟")
        ...
        for i, (group_id, group_info) in enumerate(sorted_groups[:5]):
            ...
            print(f"    {i+1}. {group_id}: GSS={gss:.3f}, 用户数={user_count}")
    ...
    def _load_product_avg_from_cache(self, cache_reader) -> Dict:
        ...
        print(f"   已计算 {len(product_avg)} 个产品的平均评分")
        return product_avg
    ...
    def save_results(self):
        ...
        print(f" 最终群组信息已保存: {final_groups_path}")
        print(f" 群组用户列表已保存: {groups_users_path} (纯Python格式)")
        print(f" 群组分析CSV已保存: {group_csv_path}")
        print(" 模块6-7完成：节点聚合和群组净化合并完毕")
    ...
    def run(self):
        ...
        try:
            ...
            if os.path.exists(final_groups_path) and os.path.exists(group_csv_path):
                print(" 检测到模块6-7缓存文件，直接加载结果...")
                # 加载缓存的结果
                with open(final_groups_path, 'rb') as f:
                    self.final_groups = pickle.load(f)
                print(f" 已加载最终群组信息: {len(self.final_groups)}个群组")
                print(f" 已加载群组分析CSV: {group_csv_path}")
                print(f" 模块6-7缓存加载完成，跳过ISS计算和群组净化")
                return True
...
class Module8_ValidationAndOutput:
    ...
    def _load_ground_truth(self):
        ...
        print(f"从虚拟节点中获取到 {len(sampled_user_ids)} 个采样用户")
        ...
        print(f" 已加载所有用户: {len(self.user_labels)} 个用户（从缓存，无SQL查询）")
        print(f"  - 水军用户: {spam_count} 个")
        print(f"  - 真实用户: {normal_count} 个")
        
    def generate_predictions(self):
        """基于GSS得分和TopK选择生成群组级别预测结果"""
        print("步骤3/4: 生成群组级别预测结果...")
        ...
        print(f"  - 总群组数: {len(self.final_groups)}")
        print(f"  - 仅使用GSS排序选取Top{self.top_k}群组")
        print(f"  - 最终Top{self.top_k}群组数: {len(self.detected_groups)}")
        ...
        print(f" 群组级别预测结果生成完成")
        print(f"  - 模型认定的水军群组数: {len(self.detected_groups)}")
        print(f"  - 真实水军群组数: {len(self.true_spam_groups)}")
        
    def calculate_metrics(self):
        """计算群组级别性能指标"""
        print("步骤4/4: 计算群组级别性能指标...")
    ...
    def calculate_multi_k_metrics(self):
        ...
        print(" 多个TopK值的性能指标计算完成")
    
    def save_multi_k_results(self):
        """保存多个TopK值的结果到专门文件夹"""
        print("保存多个TopK值的指标结果...")
        ...
        print(f" 多K值结果已保存到: {multi_k_dir}")
        print(f"  - 各K值详细指标: metrics_topk_*.json")
        print(f"  - 各K值文本报告: report_topk_*.txt")
        print(f"  - 汇总对比表: topk_comparison_summary.csv")
        print(f"  - 精确率汇总: precision_by_topk.csv")
        print(f"  - 召回率汇总: recall_by_topk.csv")
        print(f"  - F1值汇总: f1_score_by_topk.csv")
    ...
    def _print_metrics(self):
        ...
        print(f"                    预测")
        print(f"                水军群组  非水军群组")
        print(f"实际  水军群组    {cm['tp']:6d}      {cm['fn']:6d}")
        print(f"      非水军群组  {cm['fp']:6d}      {cm['tn']:6d}")
        print()
        
        print(f"群组级别性能指标:")
        print(f"  精确率 (Precision): {self.metrics['precision']:.4f}")
        print(f"  召回率 (Recall):    {self.metrics['recall']:.4f}")
        print(f"  F1值 (F1-Score):   {self.metrics['f1_score']:.4f}")
        print(f"  检测准确性:         {self.metrics['accuracy']:.4f}")
        print()
        
        print(f"群组检测统计:")
        print(f"  总群组数:                    {self.metrics['total_groups']}")
        print(f"  实际真实水军群组数:          {self.metrics['actual_spam_groups']}")
        print(f"  模型认定的水军群组数:        {self.metrics['detected_groups']}")
        print(f"  模型检测出的真实水军群组数:  {self.metrics['true_spam_groups']}")
        print()
        
        print(f"评估参数:")
        print(f"  δ_g (群组判定阈值):  {self.metrics['delta_g']}")
        print(f"  δ_G (GSS过滤阈值):   {self.metrics['delta_G']}")
    ...
    def save_results(self):
        ...
        print(f" 预测结果已保存: {prediction_csv_path}")
        print(f" 检测报告已保存: {report_path}")
        print(" 模块8完成：指标验证与结果输出完毕")
    ...
    def run(self):
        ...
        try:
            ...
            if os.path.exists(metrics_file) and os.path.exists(predictions_file) and os.path.exists(report_file):
                print(f" [模块8] 发现缓存文件，直接加载结果")
                # 加载性能指标
                with open(metrics_file, 'r', encoding='utf-8') as f:
                    metrics = json.load(f)
                print(f" [模块8] 加载性能指标: 准确率={metrics.get('accuracy', 'N/A'):.4f}, F1分数={metrics.get('f1_score', 'N/A'):.4f}")
                # 加载预测结果
                predictions_df = pd.read_csv(predictions_file)
                print(f" [模块8] 加载预测结果: {len(predictions_df)}条记录")
                # 显示报告文件信息
                with open(report_file, 'r', encoding='utf-8') as f:
                    report_lines = f.readlines()
                print(f" [模块8] 加载检测报告: {len(report_lines)}行")
                print(f" [模块8] 缓存加载完成，跳过完整执行")
                return True
            
            print(f" [模块8] 未找到缓存文件，开始完整执行")
            self.load_data()
...
class SpamGroupDetectionPipeline:
    ...
    def __init__(self, db_path="DataSet/Electronics_2013_1.6.db", sample_ratio=1.0, 
        ...
        print(f" 数据集 [{self.dataset_name}] 引力图分位数={self.attraction_pct}%, 斥力图分位数={self.repulsion_pct}%")

        # 自动检测GPU可用性，如果use_gpu为None则自动使用GPU（如果可用）
        if use_gpu is None:
            self.use_gpu = torch.cuda.is_available()
            if self.use_gpu:
                print(" 自动启用GPU加速进行GAT训练")
            else:
                print("️  GPU不可用，将使用CPU进行GAT训练")
        else:
    ...
    def run_full_pipeline(self, start_module=1, end_module=8):
        ...
        print(f"采样比例: {self.sample_ratio}")
        print(f"引力图阈值: {self.attraction_threshold}")
        print(f"斥力图阈值: {self.repulsion_threshold}")
        print(f"Lambda因子: {self.lambda_factor}")
        print(f"ISS阈值: {self.iss_threshold}")
        print(f"群组判定阈值: {self.group_threshold}")
        
        # 显示GSS配置
        if self.dataset_name == "Electronics":
            print(f"GSS配置: Electronics专属（极强行为指标）")
        elif self.dataset_name == "Clothing":
            print(f"GSS配置: Clothing专属（强化规模+极端）")
        elif self.dataset_name == "Cell_Phones":
            print(f"GSS配置: Cell_Phones专属（极强行为指标）")
        else:
            print(f"GSS配置: 默认配置（平衡权重）")
        
        print(f"执行模块: {start_module} - {end_module}")
        print("="*60)
        ...
        try:
            ...
            print(f"结果保存目录: {get_result_dir(self.sample_ratio, self.db_path)}")
            print("="*60)
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        print("=" * 60)
        print("运行配置: group_threshold=0.7, 模块1-8, 独立子进程模式")
        print("=" * 60)
        for db in DATASETS:
            print(f"\n{'='*60}")
            print(f"启动子进程: {db}")
